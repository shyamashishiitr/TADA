...
export const CelebrationOverlay = ({ message, darkMode = false }: CelebrationOverlayProps) => {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none">
      ...
      <div className="absolute inset-0 overflow-hidden">
        {[...Array(20)].map((_, i) => (
          <div
            key={i}
            className="absolute text-4xl animate-[fadeIn_0.5s_ease-out]"
            style={{
              left: `${Math.random() * 100}%`,
              top: `${Math.random() * 100}%`,
              animation: `fadeIn 0.5s ease-out ${Math.random() * 0.3}s, slideDown 2s ease-in ${Math.random() * 0.3}s`,
